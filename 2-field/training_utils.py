    ...
    def _foreach_batch(
        ...
        with tqdm.tqdm(desc=pbar_name, total=num_batches, file=pbar_file) as pbar:
            ...
            for batch_idx in range(num_batches):
                ...
                pbar.set_description(f"{pbar_name} (Loss {batch_res.loss:.5f})")
            ...
            pbar.set_description(f"(training loss {avg_loss:.5f})")

            batch_accuracy = np.array(batch_accuracy)
            if batch_accuracy.shape[1] > 1:
                avg_accuracy = batch_accuracy.mean(axis=0)
                pbar.set_description(f"(test loss {avg_loss:.4f}, UVW L1 {batch_accuracy.mean():.4f} [{avg_accuracy[0]:.4f} {avg_accuracy[1]:.4f} {avg_accuracy[2]:.4f}])")
            avg_accuracy = batch_accuracy.mean()
    ...
    def test_batch(self, batch) -> BatchResult:
        ...
        with torch.no_grad():
            ...
            with autocast(device_type="cuda"):
                ...
                y_pred = output
                Lu = torch.nn.functional.l1_loss(y_pred[:, 0], y[:, 0])
                Lv = torch.nn.functional.l1_loss(y_pred[:, 1], y[:, 1])
                Lw = torch.nn.functional.l1_loss(y_pred[:, 2], y[:, 2])
                Luvw = [Lu.item(), Lv.item(), Lw.item()]

        return BatchResult(loss=loss.item(), batch_accuracy=Luvw)