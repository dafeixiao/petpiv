...
def show_imset_with_idx(imset, title=None, rc=None):
    """
    show all the matrices (more than 1) in the imset
    :param imset: a tuple with images(ndarray or tensor)
    :param title: title of the imset
...
def mask_resipe(mask, aper_pn, mask_pn):
    # th original mask should have 0 outsied aperture
    diameter = np.sqrt((mask != 0).sum() / np.pi) * 2
    new_size = int(round(aper_pn / diameter * mask.shape[0]))
    # nearest interpolation
    mask_resized = skimage.transform.resize(mask, (new_size, new_size), order=0, anti_aliasing=False,
                                            preserve_range=True)

    if new_size >= mask_pn:  # crop
        diff = new_size - mask_pn
        before = int(diff / 2)
        mask_final = mask_resized[before:before + mask_pn, before:before + mask_pn]
    else:  # pad zeros
        diff = mask_pn - new_size
        before = int(diff / 2)
        after = diff - before
        mask_final = np.pad(mask_resized, ((before, after), (before, after)))
