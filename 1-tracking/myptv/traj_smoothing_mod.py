...
class smooth_trajectories(object):
    ...
    def smooth(self):
        ...
        for tr_num in tqdm(traj_dic.keys()):
            ...
            #
            # tr_before_sm = array(traj).T[1:4,:]
            # fig = plt.figure()
            # ax = fig.add_subplot(121, projection='3d')
            # ax.plot(tr_before_sm[0, :], tr_before_sm[1, :], tr_before_sm[2, :])
            # ax.plot(p[0], p[1], p[2])
            # ax.set_title(f'{len(p[0])}')
            # ax = fig.add_subplot(122)
            # ax.plot(v[0], color='black')
            # ax.plot(v[1], color='black')
            # ax.plot(v[2], color='r')
            # plt.show(block=True)  # wait until window is closed
            # plt.close(fig)
            ...
            N = max(int(self.window / 2), 0)
            for i in range(N, len(traj_dic[tr_num]) - N):  # why remove the head and tail???
