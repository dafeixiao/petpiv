...
class tracker_four_frames(object):
    ...
    def __init__(self, fname, mean_flow=0.0, d_max=1e10, dv_max=1e10,
                 z_weight=0.05, dz_max=0.75, dz_link_max=0.75,
    ...
    def find_nearest_neighbour(self, particle, frame_num, use_scaled=True):
        ...
        for d, idx in zip(dists, idxs):
            ...
            if abs(z_cand - z_pred) <= self.dz_max:
                return d, idx
    ...
    def find_best_estimate_link(self, particle):
        ...
        # scaled space for robustness to noisy z
        try:
            tree = self.trees_scaled[frame_num]
        except:
            tree = KDTree(self._scale_xyz_arr(self.particles[frame_num][:, 1:4]))
            self.trees_scaled[frame_num] = tree

        x_proj_s = self._scale_xyz(x_proj)

        dist = lambda p: sum((x_proj_s - self._scale_xyz(p[1:4])) ** 2) ** 0.5
        proj_neighbours = [(dist(self.particles[frame_num][i]), i) for i in
                           tree.query_ball_point(x_proj_s, self.dv_max)]


        z_i = particle[3]  # current z at frame i

        proj_neighbours = [
            (d, i) for (d, i) in proj_neighbours
            # gate 1: candidate must be close to projected z (optional but keeps BE intent)
            if abs(self.particles[frame_num][i][3] - x_proj[2]) <= self.dz_max
               # gate 2 (Option 2): candidate must also be close to current z (prevents huge-z links)
               and abs(self.particles[frame_num][i][3] - z_i) <= self.dz_link_max
        ]

        if len(proj_neighbours) == 0:
            return None
        ...
        try:
            tree = self.trees_scaled[frame_num + 1]
        except:
            tree = KDTree(self._scale_xyz_arr(self.particles[frame_num + 1][:, 1:4]))
            self.trees_scaled[frame_num + 1] = tree
        ...
        for j in range(len(proj_neighbours)):
            ...
            nnd, dump = tree.query(self._scale_xyz(xip2), k=1)
