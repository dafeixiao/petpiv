...
class initial_cal_gui(object):
    ...
    def calibrate(self):
        ...
        self.cam = camera_extendedZolof(self.cam_name, cal_points_fname = cpf)
        self.cam.load('.')
        print('camera data loaded successfully.')
        cal = calibrate_extendedZolof(self.cam, 
                                      self.cam.image_points,
                                      self.cam.lab_points,
                                      quadratic=True)
        print('solving initial calibration of the camera...')
        cal.calibrate()
        print('done!')
