...
class tracker_four_frames(object):
    ...
    def __init__(self, fname, mean_flow=0.0, d_max=1e10, dv_max=1e10,
                 z_weight=0.01, dz_link_max=0.75,
        ...
        self.dz_link_max = float(dz_link_max)
    ...
    def _scale_xyz(self, xyz):
        xyz = array(xyz, dtype=float).copy()
        xyz[2] *= self.z_weight
        return xyz

    def _scale_xyz_arr(self, xyz_arr):
        xyz_arr = array(xyz_arr, dtype=float).copy()
        xyz_arr[:, 2] *= self.z_weight
        return xyz_arr
    ...
    def track_all_frames(self, frames=None):
        ...
        for k in self.particles.keys(): N_p += len(self.particles[k])
    ...
    def track_single_frame(self, frame_num):
        ...
        for i in range(len(p1_lst)):
            ...
            cond = (best_estimate[0] < self.d_max)
            # ===================================================
            if cond:
        ...
        for i in range(len(p1_lst)):
            ...
            # use_scaled = (frame_num != self.times[0])
            # nn_val = self.find_nearest_neighbour(p1, frame_num + 1, use_scaled=use_scaled)

            # 1) try RAW NN first (good for preventing XY-close/Z-far births)
            nn_val = self.find_nearest_neighbour(p1, frame_num + 1, use_scaled=False)

            # 2) if raw didn't find a close enough match, fall back to SCALED NN
            if nn_val[0] >= self.d_max:
                nn_val = self.find_nearest_neighbour(p1, frame_num + 1, use_scaled=True)
            ...
            if nn_val[0] < self.d_max:
                ...
                # cond = True
                # Candidate in frame t+1
                idx2 = nn_val[1]
                p2 = p2_lst[idx2]

                # Mutual NN check: does p2 link back to p1?
                # Use the same raw-first then scaled-fallback logic.
                back = self.find_nearest_neighbour(p2, frame_num, use_scaled=False)
                if back[0] >= self.d_max:
                    back = self.find_nearest_neighbour(p2, frame_num, use_scaled=True)

                cond = (back[1] == i)  # i is p1's index in p1_lst
    ...
    def find_nearest_neighbour(self, particle, frame_num, use_scaled=True):
        ...
        if use_scaled:
            cache = self.trees_scaled
            pts = self._scale_xyz_arr(self.particles[frame_num][:, 1:4])
            q = self._scale_xyz(p)
        else:
            cache = self.trees_raw
            pts = self.particles[frame_num][:, 1:4]
            q = p

        try:
            tree = cache[frame_num]
        except:
            tree = KDTree(pts)
            cache[frame_num] = tree
        ...
        if self.store_candidates:
            ...
            candidates = tree.query_ball_point(q, self.d_max)
        ...
        # instead of k=1, ask for a few candidates
        dists, idxs = tree.query(q, k=min(10, len(self.particles[frame_num])))  # 10 is usually enough

        dists = array(dists, ndmin=1)
        idxs = array(idxs, ndmin=1)

        # z prediction in the target frame (includes mean flow shift)
        z_pred = p[2]  # because p = particle[1:4] + dX

        for d, idx in zip(dists, idxs):
            idx = int(idx)
            z_cand = self.particles[frame_num][idx][3]  # z column in your row
            if abs(z_cand - z_pred) <= self.dz_link_max:
                return float(d), idx
    ...
    def find_best_estimate_link(self, particle):
        """
        Best-estimate linking from t to t+1 using a t+2 consistency score.

        Key choices:
          - Projections are computed in SCALED space (x, y, z*z_weight).
          - Hard gate on actual link dz from t to t+1: |z_{t+1} - z_t| <= dz1_max
          - Scoring uses t+2 nearest neighbours to projected x_{t+2} (scaled),
            but only accepts neighbours satisfying |z_{t+2} - z_t| <= dz2_max,
            with dz2_max = 2 * dz1_max.
        """
        frame_num = particle[-1] + 1  # this is t+1
        id_ = particle[0]

        # Need previous point at t-1
        p_im1 = self.get_particle_by_id(id_, particle[-1] - 1)
        if p_im1 is None:
            return None

        dt = frame_num - particle[-1]  # usually 1

        # ---- scaled coordinates for robust kinematics ----
        xi_s = self._scale_xyz(particle[1:4])
        xim1_s = self._scale_xyz(p_im1[1:4])

        # t -> t+1 projection in scaled space (constant velocity)
        v_s = (xi_s - xim1_s) / dt
        x_proj_s = xi_s + dt * v_s

        # KDTree at t+1 in scaled space
        try:
            tree1 = self.trees_scaled[frame_num]
        except KeyError:
            tree1 = KDTree(self._scale_xyz_arr(self.particles[frame_num][:, 1:4]))
            self.trees_scaled[frame_num] = tree1

        # Candidate indices at t+1 within dv_max (scaled)
        cand_idxs = tree1.query_ball_point(x_proj_s, self.dv_max)
        if len(cand_idxs) == 0:
            return None

        # ---- dz constraints ----
        z_t = particle[3]
        dz1_max = self.dz_link_max
        dz2_max = 2.0 * dz1_max  # your requested rule

        # Keep only candidates that satisfy dz(t -> t+1)
        candidates = []
        for idx1 in cand_idxs:
            idx1 = int(idx1)
            z_t1 = self.particles[frame_num][idx1][3]
            if abs(z_t1 - z_t) <= dz1_max:
                # store also their "distance to proj at t+1" in scaled space (optional tie-break)
                x_t1_s = self._scale_xyz(self.particles[frame_num][idx1][1:4])
                d1 = float(np.linalg.norm(x_proj_s - x_t1_s))
                candidates.append((d1, idx1))

        if len(candidates) == 0:
            return None

        # If no t+2 exists, fall back to closest-to-projection at t+1
        if frame_num + 1 > self.times[-1]:
            return min(candidates, key=lambda x: x[0])

        # KDTree at t+2 in scaled space
        try:
            tree2 = self.trees_scaled[frame_num + 1]
        except KeyError:
            tree2 = KDTree(self._scale_xyz_arr(self.particles[frame_num + 1][:, 1:4]))
            self.trees_scaled[frame_num + 1] = tree2

        # Score each t+1 candidate by t+2 consistency (with dz(t -> t+2) gate)
        best_score = float("inf")
        best_idx1 = None
        best_d1 = None  # optional tie-break

        k2 = min(10, len(self.particles[frame_num + 1]))  # how many t+2 neighbours to try

        for d1, idx1 in candidates:
            # scaled x_{t+1}
            xip1_s = self._scale_xyz(self.particles[frame_num][idx1][1:4])

            # Build t+2 projection in scaled space using (t-1, t, t+1)
            # vi_s, ai_s defined in scaled space
            vi_s = (xip1_s - xim1_s) / (2.0 * dt)
            ai_s = (xip1_s - 2.0 * xi_s + xim1_s) / (dt ** 2)

            xip2_s = xi_s + 2.0 * dt * vi_s + 2.0 * (dt ** 2) * ai_s

            # Query a few nearest at t+2 (scaled), then apply dz(t -> t+2) gate
            dists2, idxs2 = tree2.query(xip2_s, k=k2)
            dists2 = np.atleast_1d(dists2)
            idxs2 = np.atleast_1d(idxs2)

            score = float("inf")
            for d2, idx2 in zip(dists2, idxs2):
                idx2 = int(idx2)
                z_t2 = self.particles[frame_num + 1][idx2][3]
                if abs(z_t2 - z_t) <= dz2_max:
                    score = float(d2)  # closest passing neighbour distance
                    break

            # Choose smallest score; optional tie-break on d1
            if (score < best_score) or (
                    score == best_score and d1 < (best_d1 if best_d1 is not None else float("inf"))):
                best_score = score
                best_idx1 = idx1
                best_d1 = d1

        if best_idx1 is None or not np.isfinite(best_score):
            return None

        # Return (score, index_at_t+1). score is in scaled units.
        return (best_score, best_idx1)
    ...
    def save_results(self, fname):
        ...
        savetxt(fname, data_to_save, delimiter='\t', fmt=fmt)
